...
export const Login = ( { toggle, proceed, onInput, action='login', name, handle, email, password, available } ) => <Card>
		...
		{ action == 'register' && <Input error={ !available && 'This handle is taken' } info={ 'Your handle is a unique username you can share with your friends so they can easily find you.' } value={ handle } onChangeText={ t => onInput( 'handle', t ) } label='username/handle' /> }
